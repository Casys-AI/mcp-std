...
import type { ToolExecutor, ExecutorConfig } from "../../src/dag/types.ts";
...
Deno.test("Checkpoint Resume Security - AIL/HIL Enforcement", async (t) => {
  ...
  await t.step("SECURITY: Resume with HIL enabled requires approval", async () => {
    ...
    for await (const event of resumeGen) {
      if (event.type === "decision_required" && event.decisionType === "HIL") {
        ...
        executor.enqueueCommand({ type: "approval_response", checkpointId: "resume", approved: true });
      }
  ...
  await t.step("Resume without HIL/AIL → executes normally (backward compatibility)", async () => {
    const config: ExecutorConfig = {
