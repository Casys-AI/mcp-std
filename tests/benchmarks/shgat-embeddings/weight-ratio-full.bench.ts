/**
 * Weight Ratio Benchmark - Full dataset (all caps with queries)
 *
 * Run: deno run --allow-all tests/benchmarks/shgat-embeddings/weight-ratio-full.bench.ts
 */
...
console.log("=== Weight Ratio Benchmark (Full Dataset) ===\n");
...
const allQueries: Query[] = (scenario.testQueries || []).filter((q: Query) => q.intentEmbedding?.length);

// Include ALL caps that have embeddings AND (have queries OR have events)
const capsInQueries = new Set(allQueries.map(q => q.expectedCapability));
const capsInEvents = new Set(allEvents.map(e => e.selectedCapability));
const relevantCapIds = new Set([...capsInQueries, ...capsInEvents]);

const caps = allCaps.filter(c => relevantCapIds.has(c.id));
const events = allEvents.filter(e => caps.some(c => c.id === e.selectedCapability));
const queries = allQueries.filter(q => caps.some(c => c.id === q.expectedCapability));
...
function evaluate(
...
): { mrr: number; hit1: number; hit3: number; hit5: number; hit10: number } {
  ...
  let hits1 = 0, hits3 = 0, hits5 = 0, hits10 = 0, mrrSum = 0, evaluated = 0;
  ...
  return {
    ...
    hit10: evaluated > 0 ? hits10 / evaluated * 100 : 0,
  };
}
...
console.log("BGE %  │ Avg MRR │  ±Std  │ Hit@1  │ Hit@3  │ Hit@5  │ Hit@10 │");
console.log("───────┼─────────┼────────┼────────┼────────┼────────┼────────┤");

interface Result { bgeWeight: number; avgMrr: number; stdMrr: number; avgHit1: number; avgHit3: number; avgHit5: number; avgHit10: number; }
const allResults: Result[] = [];

for (const bgeWeight of RATIOS) {
  const runs: { mrr: number; hit1: number; hit3: number; hit5: number; hit10: number }[] = [];
  ...
  const avgHit5 = runs.reduce((s, r) => s + r.hit5, 0) / RUNS;
  const avgHit10 = runs.reduce((s, r) => s + r.hit10, 0) / RUNS;

  allResults.push({ bgeWeight, avgMrr, stdMrr, avgHit1, avgHit3, avgHit5, avgHit10 });

  const label = bgeWeight === 0 ? "N2V" : bgeWeight === 1 ? "BGE" : `${(bgeWeight * 100).toFixed(0)}`;
  console.log(
    `${label.padStart(5)}% │ ${avgMrr.toFixed(3).padStart(7)} │ ${stdMrr.toFixed(3).padStart(6)} │ ${(avgHit1.toFixed(1) + "%").padStart(6)} │ ${(avgHit3.toFixed(1) + "%").padStart(6)} │ ${(avgHit5.toFixed(1) + "%").padStart(6)} │ ${(avgHit10.toFixed(1) + "%").padStart(6)} │`
  );
}
...
console.log(`\n🏆 Best Config: BGE=${(best.bgeWeight * 100).toFixed(0)}% / N2V=${((1 - best.bgeWeight) * 100).toFixed(0)}%`);
console.log(`   MRR = ${best.avgMrr.toFixed(3)} ± ${best.stdMrr.toFixed(3)}`);
console.log(`   Hit@1=${best.avgHit1.toFixed(1)}% Hit@3=${best.avgHit3.toFixed(1)}% Hit@5=${best.avgHit5.toFixed(1)}% Hit@10=${best.avgHit10.toFixed(1)}%`);

console.log(`\n📊 Comparisons:`);
console.log(`   BGE only:  MRR=${bgeOnly.avgMrr.toFixed(3)} Hit@10=${bgeOnly.avgHit10.toFixed(1)}%`);
console.log(`   N2V only:  MRR=${n2vOnly.avgMrr.toFixed(3)} Hit@10=${n2vOnly.avgHit10.toFixed(1)}%`);
console.log(`   Best vs BGE: ${((best.avgMrr / bgeOnly.avgMrr - 1) * 100).toFixed(0)}%`);
console.log(`   Best vs N2V: ${((best.avgMrr / n2vOnly.avgMrr - 1) * 100).toFixed(0)}%`);

// Random baseline for reference
const randomMRR = 1 / caps.length;
console.log(`\n📈 vs Random (MRR=${randomMRR.toFixed(4)}):`);
console.log(`   Best is ${(best.avgMrr / randomMRR).toFixed(1)}x random`);
console.log(`   BGE is ${(bgeOnly.avgMrr / randomMRR).toFixed(1)}x random`);
