...
export class CapabilityStore {
  ...
  private dependencyStore: CapabilityDependencyStore;
  ...
  constructor(
    ...
    // Initialize dependency store (extracted for separation of concerns)
    this.dependencyStore = new CapabilityDependencyStore(db);
      ...
      for (const taskNode of taskNodes) {
        ...
        try {
          ...
          if (calledCapabilities.length > 0) {
            ...
            // Create "contains" edge (delegated to dependency store)
            await this.dependencyStore.addDependency({
  ...
  // ============================================
  // Capability Dependency Methods (delegated to CapabilityDependencyStore)
  // ============================================

  /**
   * Get the dependency store for direct access
   */
  getDependencyStore(): CapabilityDependencyStore {
    return this.dependencyStore;
  }

  /**
   * Add a dependency relationship between two capabilities
   * @deprecated Use getDependencyStore().addDependency() for new code
   */
  async addDependency(input: CreateCapabilityDependencyInput): Promise<CapabilityDependency> {
    return this.dependencyStore.addDependency(input);
  }

  /**
   * Get dependencies for a capability
   * @deprecated Use getDependencyStore().getDependencies() for new code
   */
  async getDependencies(
    capabilityId: string,
    direction: "from" | "to" | "both" = "both",
  ): Promise<CapabilityDependency[]> {
    return this.dependencyStore.getDependencies(capabilityId, direction);
  }

  /**
   * Get all capability dependencies (for graph sync)
   * @deprecated Use getDependencyStore().getAllDependencies() for new code
   */
  async getAllDependencies(minConfidence = 0.3): Promise<CapabilityDependency[]> {
    return this.dependencyStore.getAllDependencies(minConfidence);
  }

  /**
   * Get count of dependencies for a capability
   * @deprecated Use getDependencyStore().getDependenciesCount() for new code
   */
  async getDependenciesCount(capabilityId: string): Promise<number> {
    return this.dependencyStore.getDependenciesCount(capabilityId);
  }

  /**
   * Remove a dependency relationship
   * @deprecated Use getDependencyStore().removeDependency() for new code
   */
  async removeDependency(fromId: string, toId: string): Promise<void> {
    return this.dependencyStore.removeDependency(fromId, toId);
  }
  ...
  > {
    ...
    const enhancedResults = await Promise.all(
      results.map(async ({ capability, semanticScore }) => {
        const deps = await this.dependencyStore.getDependencies(capability.id, "from");
        const dependencyDeps = deps.filter((d) => d.edgeType === "dependency");

        const dependencies: Capability[] = [];
        for (const dep of dependencyDeps.slice(0, 10)) {
          const depCap = await this.findById(dep.toCapabilityId);
          if (depCap) {
            dependencies.push(depCap);
          }
        }

        return { capability, semanticScore, dependencies };
      }),
    );
